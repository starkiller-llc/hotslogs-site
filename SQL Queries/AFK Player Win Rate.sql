...
q3.GamesPlayedTeamLeague, '|',
format(q3.WinPercentWithDisconnectedPlayerQuickMatch * 100, 1), '%|',
format(q3.WinPercentWithDisconnectedPlayerHeroLeague * 100, 1), '%|',
format(q3.WinPercentWithDisconnectedPlayerTeamLeague * 100, 1), '%') as RedditFormat
from (select
...
sum(case when q2.GameMode = 5 then 1 else 0 end) as GamesPlayedTeamLeague,
...
sum(case when q2.GameMode = 5 then q2.IsWinner else 0 end) / sum(case when q2.GameMode = 5 then 1 else 0 end) as WinPercentWithDisconnectedPlayerTeamLeague
from (select
q.ReplayID,
q.PlayerID,
q.GameMode,
...
where r.TimestampReplay > date_add(now(), interval -15 day) and r.TimestampReplay < now() and r.GameMode in (3,4,5)
